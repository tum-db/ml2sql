...
RUN apt-get update
# Install some tools
RUN apt-get install -y cmake g++ wget uuid-dev unzip pkgconf git python xxd
## Install antlr
RUN cd /tmp && wget http://www.antlr.org/download/antlr4-cpp-runtime-4.7.1-source.zip && wget http://www.antlr.org/download/antlr-4.7.1-complete.jar  && \
   ...
   cd antlr4-cpp-runtime && mkdir build && mkdir run && cd build && cmake -DCMAKE_BUILD_TYPE=Release .. && make -j && make install && \
...
RUN cd /tmp/ && git clone https://github.com/oktal/pistache.git && cd pistache && git submodule update --init && \
   mkdir build && cd build && cmake -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Release .. && make -j && make install && \
   LD_LIBRARY_PATH=/usr/local/lib && export LD_LIBRARY_PATH && ldconfig -v
...
# Install ml2sql
RUN cd ./ml2sql && rm -rf build && mkdir build && cd ./build && cmake .. && make -j
# Run
EXPOSE 5000
